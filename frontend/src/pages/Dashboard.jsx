...
const riskLabel = (value) => (value >= 0.7 ? "High" : value >= 0.4 ? "Medium" : "Low");
const riskStatus = (value) => (value >= 0.7 ? "Critical" : value >= 0.4 ? "Warning" : "Healthy");
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetchPredictions();
  }, [csvUploaded, setPredictions, active]);

  // Fetch metrics for active machine
  useEffect(() => {
    if (!active) return;
    let mounted = true;
  ...
  // Active machine info for prediction cards
  const activeInfo = useMemo(() => {
    const last = series.at(-1) || {};
    const temp = last?.temp?.toFixed?.(1) ?? 0;
    const vib = last?.vib?.toFixed?.(3) ?? 0;
  ...
  // Handle live simulation updates from Simulation.jsx
  const handleSimulationUpdate = ({ machineId, temp, vib, risk, confidence, impact, suggestion }) => {
    setSeries((prev) => [...prev.slice(-59), { time: toTime(Date.now()), temp, vib }]);
    setPredictions((prev) => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 space-y-8">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Chart title={`Live Metrics — ${active}`} data={series} className="md:col-span-2" />
        <PredictionCard
          machine={active}
          risk={activeInfo.risk}
          confidence={activeInfo.confidence}
          impact={activeInfo.impact}
          suggestion={activeInfo.suggestion}
        />
      </div>
      ...
      <div className="mt-8">
        <Simulation
          ...
          activeMetrics={activeInfo}
          onSimulate={handleSimulationUpdate}
        />
      </div>
