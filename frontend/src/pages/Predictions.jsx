...
export default function Predictions() {
  const { csvUploaded, machines, handleCsvUpload } = useAppContext();
  ...
  // If no CSV uploaded, show upload message + file input
  if (!csvUploaded.length) {
    return (
      <motion.div
        className="mx-auto max-w-3xl text-slate-300 space-y-3 p-6"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
        <h2 className="text-3xl font-bold">Predictions</h2>
        <p>
          Upload historical machine data CSV first to enable predictive maintenance features.{" "}
          <strong>Predictions use pre-trained model.</strong>
        </p>
        <input
          type="file"
          accept=".csv"
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-[#1a1c24] p-6 rounded-xl shadow-lg">
        ...
        <div className="md:col-span-3 flex justify-end mt-2">
          <button
            type="submit"
            disabled={loading}
            className={`btn bg-blue-600 hover:bg-blue-500 text-white font-semibold px-6 py-2 rounded-lg flex items-center gap-2 transition-all duration-300 ${
